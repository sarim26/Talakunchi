...
export const waybackUrlsTool = {
    ...
    description: "Backup endpoint discovery: query the Wayback CDX for historical URLs of the target host. Usually empty for private IPs (RFC1918) and hosts never archived on the public web.",
    ...
    handler: async (input, emit) => {
        ...
        for (const raw of r.stdout.split("\n")) {
            ...
            if (normHost(url.hostname) !== normHost(host))
                continue;
            const href = url.href;
            if (seen.has(href))
                continue;
        ...
        const noRowsHint = facts.length === 0
            ? privateIp
...
function isPrivateOrReservedIp(host) {
    ...
    if (oct.some((n) => !Number.isFinite(n) || n > 255))
        return false;
    const [a, b] = oct;
    if (a === 10)
        return true;
