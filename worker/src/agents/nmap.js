...
export const nmapTool = {
    ...
    handler: async (input, emit) => {
        const profile = String(input.args?.profile ?? "deep");
        const portsArg = input.args?.ports;
        const extraArgsRaw = String(input.args?.extraArgs ?? "").trim();
        ...
        for (const svc of openServices)
            emit.fact({ type: "service", value: svc, source: "nmap" });
        return {
            status: "succeeded",
            durationMs: r.durationMs,
            artifacts: { commands: [r.command], stdoutSnippet: snippet(r.stdout), stderrSnippet: snippet(r.stderr) },
...
function severityForPort(port) {
    ...
    if ([3389, 5985, 5986, 5900, 5901, 16992, 16993].includes(port))
        return "medium";
    // SMB is often a big deal in internal networks.
    if ([445, 139].includes(port))
        return "medium";
    // Databases / data stores exposed.
    if ([3306, 5432, 1433, 1521, 27017, 6379, 9200, 9300, 11211].includes(port))
        return "medium";
    // Cleartext / legacy protocols (credential leakage risk).
    if ([21, 23, 25, 110, 143, 445, 139].includes(port))
        return "medium";
