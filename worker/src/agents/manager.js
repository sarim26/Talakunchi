...
export async function decideNextAction(server, ctx, signal) {
    ...
    if (missingTool &&
        ...
        if (lastFail.missingToolInstallCommand)
            installArgs.installCommand = lastFail.missingToolInstallCommand;
        return {
            action: "invoke",
 ...
 */
function fallbackInitialNmap(server, ctx) {
    if (!server.has("recon.nmap"))
        return null;
    const history = ctx.invocationHistory ?? [];
    if (history.some((h) => h.tool === "recon.nmap"))
        return null;
    if ((ctx.knownPorts ?? []).length > 0 || (ctx.knownServices ?? []).length > 0)
        return null;
    const hints = ctx.runHints;
    const profile = hints?.initialNmapProfile === "fast" ||
        ...
        : "deep";
    const args = { profile };
    if (hints?.initialNmapPorts?.length)
        args.ports = hints.initialNmapPorts;
    if (hints?.initialNmapExtraArgs?.trim())
        args.extraArgs = hints.initialNmapExtraArgs.trim();
    return {
        action: "invoke",
        tool: "recon.nmap",
        intentGoal: `Discover open ports and services on ${ctx.targetHost} (${profile} scan)`,
        args,
        reasoning: "Automatic fallback: manager LLM unavailable; starting with recon.nmap (no ports or services known yet)."
    };
}
function coverageFirstDecision(server, ctx) {
    ...
    const nameOf = (s) => `${s.product ?? ""} ${s.name ?? ""}`;
    const infraCoverage = [
...
function verificationFirstDecision(server, ctx) {
    for (const v of ctx.pendingVerifications ?? []) {
        if (!v.verifierTool || !server.has(v.verifierTool))
            continue;
        return {
            action: "invoke",
            tool: v.verifierTool,
...
async function tryLlmDecision(server, ctx, signal) {
    const systemMsg = [
    ...
    for (let i = 0; i < attempts.length; i += 1) {
        ...
        try {
            ...
            const normalized = normalizeManagerDecisionRaw(r.value ?? safeParseJson(r.raw));
            ...
            lastManagerOllamaError = e.message;
...
function normalizeManagerDecisionRaw(value) {
    ...
    if (typeof action === "string")
        o.action = action.toLowerCase().trim();
    return o;
}
function resolveRegisteredTool(server, tool) {
    const t = tool.trim();
    if (!t)
        return null;
    if (server.has(t))
...
function fallbackFromRecommendations(server, ctx) {
    ...
    for (const p of list) {
        ...
        return {
            ...
            args: p.args ?? {},
            reasoning: "Automatic fallback: manager model produced no usable JSON after retries; running highest-priority recommendation queued by a prior tool."
        };
    }
    return null;
}
function recommendationHasRunnableArgs(p) {
    const a = p.args ?? {};
    if (p.agent === "recon.gobuster") {
        ...
        if (Array.isArray(a.http_targets) && a.http_targets.some((x) => typeof x === "string" && /^https?:\/\//i.test(x.trim())))
            return true;
        if (typeof a.targetUrl === "string" && /^https?:\/\//i.test(a.targetUrl.trim()))
            return true;
        return false;
    }
    if (p.agent === "recon.ffuf")
        return typeof a.targetUrl === "string" && /FUZZ/.test(a.targetUrl);
    return true;
}
/** Keep manager prompts small: long URL lists after spider blow up JSON size and break small models. */
function truncateUrlForPlanner(url, max = 140) {
    if (url.length <= max)
        return url;
    return `${url.slice(0, Math.max(0, max - 1))}…`;
}
function buildManagerUserPayload(server, ctx, mode) {
