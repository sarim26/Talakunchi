...
import {
  censysHost,
  enabledConnectors,
  securityTrailsPassiveDns,
  shodanHost,
  type OsintHost
} from "../osint/connectors.js";

/** Upsert an OSINT-derived asset into recon_assets (shared with the classic pipeline). */
async function persistReconAsset(targetId: string, assetType: string, value: string, source: string, confidence: number, metadata: Record<string, unknown>): Promise<void> {
  await withClient(async (c) => {
    await c.query(
      ...
      [targetId, assetType, value, source, confidence, JSON.stringify(metadata)]
    );
  });
}
 ...
 */
export const osintTool: ToolDefinition = {
  name: "recon.osint",
 ...
 */
export const passiveDnsTool: ToolDefinition = {
...
export const osintTools: ToolDefinition[] = [osintTool, passiveDnsTool];
