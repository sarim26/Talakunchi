 ...
 */
export const toolInstallerTool = {
    name: "system.tool_installer",
    description: "Install a missing CLI on the remote SSH/Kali tools host. Pass args.tool (binary to verify). Optionally pass args.installCommand: a full shell snippet to run when apt is wrong (e.g. Katana: install Go then `go install github.com/projectdiscovery/katana/cmd/katana@latest`; use `$SUDO` for privileged steps). If installCommand is omitted, runs apt-get update && apt-get install -y <tool>.",
    ...
    argSchema: {
        ...
        installCommand: {
            type: "string",
            description: "Optional shell script/commands to run on the remote host (omit to use apt-get install -y <tool>). Use $SUDO for privileged steps; PATH should include GOPATH/bin if using go install."
        }
    },
...
function buildAptInstallScript(tool, sudoPass) {
    return [
        `set +e`,
...
function buildCustomInstallScript(tool, sudoPass, installCommand) {
