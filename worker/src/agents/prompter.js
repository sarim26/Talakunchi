...
function wordlistLines(input) {
    if (!input.wordlistCatalog?.rootExists)
        return [];
    ...
    if (!wantsWeb && !wantsDns && !wantsCreds)
        return [];
    const top = (list, n = 6) => list.slice(0, n).map((e) => `  - ${e.label} → ${e.path}`).join("\n");
    const lines = ["", "Wordlists available on tools host (under /home/kali/Desktop/SecLists/):"];
    if (wantsWeb) {
