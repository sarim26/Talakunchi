...
function matchedPorts(input, predicate, defaults) {
...
function openPortsFromNmapText(stdout, protocol) {
    const out = new Set();
...
function corroboration(host, port, protocol, tool, evidence) {
    return {
...
function nseSection(stdout, scriptId) {
...
export const snmpEnumTool = {
    ...
    handler: async (input, emit) => {
        ...
        for (const block of blocks) {
            ...
            if (/iso\.|\bSTRING\b|\bOID\b|::/.test(block) && /1\.3\.6\.1/.test(block) === false && /=\s/.test(block) === false)
                continue;
            if (/=\s/.test(block)) {
                anyReadable = true;
                findings.push({
        ...
        if (anyReadable)
            findings.push(corroboration(host, 161, "udp", "snmp", "snmpwalk returned OIDs"));
        return {
            status: "succeeded",
            durationMs: r.durationMs,
            artifacts: { commands: r.commands, stdoutSnippet: snippet(r.stdout), stderrSnippet: snippet(r.stderr) },
...
export const ldapEnumTool = {
    ...
    defaultTimeoutMs: 4 * 60 * 1000,
    handler: async (input, emit) => {
        const ports = matchedPorts(input, (s) => s.protocol === "tcp" && ([389, 636, 3268, 3269].includes(s.port) || /ldap/i.test(s.name ?? "")), [389]);
