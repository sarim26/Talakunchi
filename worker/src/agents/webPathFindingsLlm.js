...
export async function findingsFromWebFactsLlm(opts) {
    ...
    try {
        ...
        for (const f of parsed.data.findings) {
            ...
            if (!matchedUrl)
                continue;
            const baseFp = f.fingerprint?.trim() ||
        ...
        opts.emitLog?.(`[web-findings-llm] ${e.message}`);
        return {
            findings: [],
            meta: {
                modelUsed: model,
...
function extractObservations(facts) {
    const out = [];
    for (const f of facts) {
        if (f.type !== "web_path" && f.type !== "web_url")
            continue;
        const v = (f.value ?? {});
        const url = typeof v.url === "string" ? v.url.trim() : "";
        if (!url)
            continue;
        const httpStatus = typeof v.status === "number"
            ? v.status
...
function longestUrlContainedIn(evidence, urls) {
    let best = null;
    for (const u of urls) {
        const variants = new Set([u, u.replace(/\/+$/, ""), u.endsWith("/") ? u : `${u}/`]);
        let hit = false;
...
function statusRank(s) {
    ...
    if (s === 200)
        return 3;
    if (s === 301 || s === 302 || s === 307 || s === 308)
        return 4;
    if (s === 204)
        return 8;
    return 6;
}
function prioritizeObservations(rows) {
...
function pathSignalScore(rawUrl) {
    let s = 0;
    try {
        const u = new URL(rawUrl);
        const path = u.pathname;
        const search = u.search;
        if (/[?&]c=[a-z];o=[a-z]/i.test(search))
            s += 95;
        const depth = urlPathDepth(rawUrl);
        if (depth <= 1 && /\.php$/i.test(path))
            s += 55;
        else if (depth <= 2 && /\.php$/i.test(path))
            s += 40;
        if (/(^|\/)phpmyadmin(\/|$)/i.test(path))
            s += depth <= 3 ? 45 : depth <= 6 ? 28 : 12;
        if (/(^|\/)drupal(\/|$)/i.test(path))
            s += depth <= 3 ? 42 : depth <= 6 ? 26 : 12;
        if (/(^|\/)chat(\/|$)/i.test(path))
            s += depth <= 3 ? 38 : 20;
        if (/(^|\/)(wp-admin|wp-login|server-status|\.git|\.env|graphql|swagger)(\/|$)/i.test(path))
            s += 50;
        if (/\.(js|mjs|css|map|woff2?|ttf|eot|png|jpe?g|gif|svg|ico)(\?|$)/i.test(path))
            s -= 35;
        if (/\/(js|css|misc|themes|jquery|img|images|fonts|vendor)\//i.test(path))
            s -= 25;
        if (/\/jquery[^/]*\.js/i.test(path))
            s -= 20;
    }
    catch {
        return 0;
...
function fingerprintTitlePart(title) {
    const t = title
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, "-")
        .replace(/^-+|-+$/g, "")
