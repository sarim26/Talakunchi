 ...
 */
export const DEFAULT_HEURISTICS: CompiledHeuristic[] = [
...
export type LoadHeuristicsResult =
  | { source: "file"; path: string; heuristics: CompiledHeuristic[] }
...
export async function loadCveHeuristics(opts?: { path?: string }): Promise<LoadHeuristicsResult> {
  ...
  try {
    ...
    const compiled: CompiledHeuristic[] = [];
    ...
    return { source: "default", path: p, heuristics: DEFAULT_HEURISTICS, error: (e as Error).message };
  }
}
