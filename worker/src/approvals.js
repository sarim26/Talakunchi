...
export async function requestApproval(input) {
    return withClient(async (c) => {
        const res = await c.query(`insert into command_approvals (scan_run_id, agent_run_id, tool, command, reasoning, impact, status, args, created_at)
...
export async function waitForApproval(id, signal) {
...
export function isLhostAllowed(lhost) {
    const v = (lhost ?? "").trim();
    if (!v)
        return true;
