...
export async function upsertNeo4jTarget(target) {
...
async function clearTargetSubgraph(s, targetId) {
    await s.run(`
...
export async function rebuildNeo4jForTarget(targetId) {
    const data = await withClient(async (c) => {
        const tRes = await c.query(`select id, name, address from targets where id = $1`, [targetId]);
        const sRes = await c.query(`select id, port, protocol, service_name, product, version from services where target_id = $1`, [targetId]);
        const fRes = await c.query(`select id, service_id, title, severity, status from findings where target_id = $1`, [targetId]);
