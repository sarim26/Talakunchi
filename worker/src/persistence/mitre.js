...
export function mitreTechniquesFor(finding) {
    const out = new Set();
    if (finding.claimType && CLAIM_TYPE_TECHNIQUES[finding.claimType]) {
        for (const t of CLAIM_TYPE_TECHNIQUES[finding.claimType])
            out.add(t);
    }
    for (const k of KEYWORD_TECHNIQUES) {
        if (k.re.test(finding.title))
            for (const t of k.techniques)
                out.add(t);
    }
    return [...out];
}
