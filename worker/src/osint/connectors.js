...
export async function shodanHost(ip, signal) {
    if (!env.SHODAN_API_KEY)
        return null;
    const data = await getJson(`https://api.shodan.io/shodan/host/${encodeURIComponent(ip)}?key=${env.SHODAN_API_KEY}`, undefined, signal);
    if (!data)
        return null;
    return {
        ip,
        ports: Array.isArray(data.ports) ? data.ports.filter((p) => typeof p === "number") : [],
        hostnames: Array.isArray(data.hostnames)
            ? data.hostnames.filter((h) => typeof h === "string")
            : [],
...
export function censysEnabled() {
    return Boolean(env.CENSYS_API_ID && env.CENSYS_API_SECRET);
}
export async function censysHost(ip, signal) {
    if (!env.CENSYS_API_ID || !env.CENSYS_API_SECRET)
        return null;
    const auth = Buffer.from(`${env.CENSYS_API_ID}:${env.CENSYS_API_SECRET}`).toString("base64");
    const data = await getJson(`https://search.censys.io/api/v2/hosts/${encodeURIComponent(ip)}`, { headers: { authorization: `Basic ${auth}` } }, signal);
    ...
    return {
        ip,
        ports: services.map((s) => s.port).filter((p) => typeof p === "number"),
        hostnames: Array.isArray(result.dns?.names)
            ? result.dns.names.filter((h) => typeof h === "string")
            : [],
...
export function securityTrailsEnabled() {
    return Boolean(env.SECURITYTRAILS_API_KEY);
}
export async function securityTrailsPassiveDns(hostname, signal) {
    if (!env.SECURITYTRAILS_API_KEY)
        return null;
    const data = await getJson(`https://api.securitytrails.com/v1/history/${encodeURIComponent(hostname)}/dns/a`, { headers: { apikey: env.SECURITYTRAILS_API_KEY } }, signal);
    const records = data?.records;
    if (!Array.isArray(records))
        return null;
    return records.slice(0, 25).map((rec) => ({
        hostname,
        ips: Array.isArray(rec.values)
            ? rec.values.map((v) => v.ip).filter((ip) => typeof ip === "string")
            : [],
